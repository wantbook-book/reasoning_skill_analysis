...
class SAETrainer(Generic[T_TRAINING_SAE, T_TRAINING_SAE_CONFIG]):
    ...
    def _train_step(
        ...
        if (self.n_training_steps + 1) % self.cfg.feature_sampling_window == 0:
            if self.cfg.logger.log_to_wandb:
                ...
                swanlab.log(sparsity_log_dict, step=self.n_training_steps)
        ...
        if (self.n_training_steps + 1) % (
            ...
            * self.cfg.logger.eval_every_n_wandb_logs
            ...
            for key, value in self.sae.log_histograms().items():
                # swanlab can directly log histograms from numpy arrays or tensors
                eval_metrics[key] = value.cpu().numpy() if isinstance(value, torch.Tensor) else value
    ...
    def _build_sparsity_log_dict(self) -> dict[str, Any]:
        ...
        sparsity_histogram = log_feature_sparsity.numpy()
        return {
            "metrics/mean_log10_feature_sparsity": log_feature_sparsity.mean().item(),
            "plots/feature_density_line_chart": sparsity_histogram,
