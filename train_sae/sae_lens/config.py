...
@dataclass
class LoggingConfig:
    # Logging config (supports swanlab)
    log_to_wandb: bool = True  # Keep field name for compatibility; using swanlab in practice
    log_activations_store_to_wandb: bool = False
    log_optimizer_state_to_wandb: bool = False
    wandb_project: str = "sae_lens_training"  # swanlab project name
    wandb_id: str | None = None  # swanlab logdir
    run_name: str | None = None  # swanlab experiment_name
    wandb_entity: str | None = None  # swanlab workspace
    ...
    def log(
        ...
        # swanlab does not support Artifacts; checkpoints are saved locally
        # swanlab.log() only supports numeric types, so we log via print here
        from sae_lens import logger
        sae_name = trainer.sae.get_name().replace("/", "__")
        logger.info(f"Checkpoint saved: {sae_name}")
...
class LanguageModelSAERunnerConfig(Generic[T_TRAINING_SAE_CONFIG]):
    ...
    dataset_path: str = ""
    data_files: list[str] | None = None  # Support loading from local files
    ...
    def __post_init__(self):
        ...
        if unique_id is None:
            import uuid
            unique_id = uuid.uuid4().hex[:8]  # Generate an 8-char unique ID
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # Handle torch.dtype values in model_from_pretrained_kwargs
        if d.get("model_from_pretrained_kwargs"):
            for key, value in d["model_from_pretrained_kwargs"].items():
                if isinstance(value, torch.dtype):
                    d["model_from_pretrained_kwargs"][key] = str(value).replace("torch.", "")
